 ...
 * Return: Returns 0 if there are no num in str, converted num otherwise,
 *         and -1 on error.
 */
int _erratoi(char *s)
{
	...
	if (*s == '+')
		s++; /* TODO: Chech why change affects the return val of main() */
	for (i = 0; s[i] != '\0'; i++)
	{
		if (s[i] >= '0' && s[i] <= '9')
		{
 ...
 * @flags: Flags indicating special handling.
